...
def visualize_clean_grid(num_samples=5, num_paths_per_sample=20):
    """
    生成清晰的网格可视化
    
    Args:
        num_samples: 评估多少个测试样本
    ...
    for sample_idx, key in enumerate(unique_keys):
        print(f"处理样本 {sample_idx + 1}/{num_samples}: {key}")
        ...
        fig.suptitle(f'Sample {sample_idx}: {key} - {num_paths_per_sample} Generated Paths', 
                     fontsize=16, y=0.995)
...
if __name__ == '__main__':
    visualize_clean_grid(num_samples=5, num_paths_per_sample=20)
